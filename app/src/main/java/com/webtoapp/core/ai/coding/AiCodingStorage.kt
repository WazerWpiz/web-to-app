        ...
        return try {
            ...
            files.forEach { file ->
                ...
                if (targetFile.exists() && !config.overwrite) {
                    throw Exception(Strings.aiCodingFileAlreadyExists.format(file.name))
                }
    ...
    private fun parseCodeBlocksByContent(response: String, codeBlocks: MutableList<CodeBlock>) {
        ...
        if (htmlContent == null) {
            ...
            if (doctypeIndex >= 0) {
                ...
                val endIndex = when {
                    endHtmlIndex >= 0 -> endHtmlIndex + 7
                    endBodyIndex >= 0 -> endBodyIndex + 7
                    lastScriptEndIndex >= doctypeIndex -> lastScriptEndIndex + 9
    ...
    fun mergeToSingleHtmlLegacy(codeBlocks: List<CodeBlock>): String {
        ...
        if (mainHtml != null) {
            ...
            val hasInlineScript = inlineScriptMatches.any { match ->
                ...
                scriptContent.isNotEmpty() && scriptContent.length > 5
            }
